...
def test_stuck0_has_invariant() -> None:
    ...
    init: s=0 → Inv. cons: Inv(s) ∧ s'=s → Inv(s'). safe: Inv → ¬s = ¬bad.
    """
    f = encode_indinv(parse_seq_aag(STUCK0))
    assert is_true(f, budget=5_000_000) is True


def test_toggle_no_invariant() -> None:
    """s'=¬s, reset=0, bad=s. Reachable at step 1 ⇒ no Inv ⇒ UNSAT."""
    f = encode_indinv(parse_seq_aag(TOGGLE))
    assert is_true(f, budget=5_000_000) is False


def test_counter2_no_invariant() -> None:
    """bad reachable ⇒ UNSAT. (8 universals, ~14 existentials.)"""
    f = encode_indinv(parse_seq_aag(COUNTER2))
    # Skolem space too large for brute force; use hqs.
    assert _hqs_solve(f) is False
...
def test_mutex_safe_vs_buggy_hqs() -> None:
    ...
    assert _hqs_solve(encode_indinv(parse_seq_aag(safe))) is True
    assert _hqs_solve(encode_indinv(parse_seq_aag(bug))) is False
...
def test_comment_header() -> None:
    f = encode_indinv(parse_seq_aag(STUCK0), source="stuck0.aag")
