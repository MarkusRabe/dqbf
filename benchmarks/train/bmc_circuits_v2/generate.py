"""Second batch of BMC circuit benchmarks (multi-op compositions).
...
@click.command()
@click.option("--out", type=click.Path(), default="benchmarks/train/bmc_circuits_v2")
def main(out: str) -> None:
    base = Path(out)
    total = 0
    for name, fn in sorted(REGISTRY_V2.items()):
        d = base / name
        ...
        for n in WIDTHS:
            ...
            for k in BOUNDS:
                ...
                with gzip.open(d / f"{inst}.dqdimacs.gz", "wt") as fp:
                    fp.write(f"c bmc_circuits_v2/{name} N={n} k={k}: {comment}\n")
                    fp.write(dqdimacs.dumps(f))
                manifest.append(
                    {
                        "path": f"{inst}.dqdimacs.gz",
                        "expected": "unknown",
                        "tags": ["bmc_circuits_v2", name],
                        "params": {"N": n, "k": k},
                    }
                )
