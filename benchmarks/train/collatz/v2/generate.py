...
def encode_tonly(n: int, k: int, step: str) -> Formula:
    ...
    return c.formula(
        comments=(f"collatz/v2 enc=tonly step={step} N={n} K={k}",)
    )
...
def encode_succinct(n: int, k: int, step: str) -> Formula:
    ...
    return c.formula(
        comments=(f"collatz/v2 enc=succinct step={step} N={n} K={k}",)
    )
...
@click.command()
@click.option("--out", type=click.Path(), default="benchmarks/train/collatz/v2")
...
def main(out: str, steps: str) -> None:
    ...
    for enc in ("unrolled", "succinct", "tonly"):
        for sv in step_variants:
            for n, k in _GRID[enc]:
                ...
                if enc == "unrolled":
                    f = _compile_eqfob(eqfob_unrolled(n, k, sv))
                else:
                    f = _DIRECT[enc](n, k, sv)
                if f.n_vars > VAR_CAP:
                    skipped.append(f"{name} ({f.n_vars} vars)")
                    continue
    ...
    for sv in step_variants:
        for n, m in GRID_INDINV:
            name = f"collatz_indinv_{sv}_n{n:02d}_m{m:02d}"
            ...
            manifest.append(
                {
                    "path": f"{name}.dqdimacs.gz",
                    "expected": "unknown",
                    "tags": ["collatz", "v2", "indinv", sv],
                    "params": {"N": n, "M": m, "encoding": "indinv", "step": sv},
                }
            )
            print(f"{name}: {f.n_vars} vars, {len(f.clauses)} clauses")

    (base / "manifest.json").write_text(json.dumps(manifest, indent=2))
    print(f"\nwrote {len(manifest)} instances to {base}/")
    if skipped:
